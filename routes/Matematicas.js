...
router.get('/readings', async (req, res, next) => {
	try {
		const readings = await service.getAllReadings('Matematicas');
...
router.get('/quizzes', async (req, res, next) => {
	try {
		const quizes = await service.getAllQuizzes('Matematicas');
...
router.get('/videos/', async (req, res, next) => {
	try {
		...
		if (q) {
			...
			const videos = await service.getVideos('Matematicas Primaria Temas');
			res.status(200).json(videos);
		}
	} catch (error) {
		next(error);
