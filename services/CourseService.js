...
class CourseService {
	...
	async getVideos(course) {
		const baseApiUrl = 'https://www.googleapis.com/youtube/v3';
		const url = `${baseApiUrl}/search?key=${process.env.APIKEY_YOUTUBE}&type=video&part=snippet&q=${course}`;
		const response = await axios.get(url);
		const results = response.data.items.map((item) => ({
			url: `https://youtu.be/${item.id.videoId}`,
			titulo: item.snippet.title,
	...
	async getAllReadings(course) {
		const querySnapshot = await db.collection('Readings').where('categoria', '==', course).get();
		if (querySnapshot.empty) {
			throw new Error('No hay readings de este Curso');
		}
	...
	async getAllQuizzes(course) {
		const querySnapshot = await db.collection('Quizzes').where('categoria', '==', course).get();
		if (querySnapshot.empty) {
			throw new Error('No hay quizes de este Curso');
		}
		let dataCollection = [];
		dataCollection = querySnapshot.docs.map((doc) => ({
			id: doc.id,
